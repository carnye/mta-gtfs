...
Mta.prototype.schedule = function(stopId, feedId) {
    ...
    _.forEach(stopIds, function(stop) {
        schedule[stop] = { N: [], S: [] };
    });
    ...
    return fetch(feedUrl)
        ...
        .then(function(data) {
            ...
            data.entity.map(function(t) {
                if (_.isEmpty(t.tripUpdate)) {
                    return;
                }

                t.tripUpdate.stopTimeUpdate.map(function(s) {
                    _.forEach(stopIds, function(stop) {
                        if (s.stopId.indexOf(stop) > -1) {
                            direction = s.stopId.replace(stop, '');
                            obj = utils.parseObj(t.tripUpdate.trip, s);
